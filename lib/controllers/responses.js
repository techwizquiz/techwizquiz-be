...
export default Router() 
  ...
  .patch('/api/v1/responses/:userId/:questionId', async (req, res) => {
    try {
      const response = await Response.PatchIncorrectAnswerToCorrect(req.params.userId, req.params.questionId);
      res.send(response);
    } catch(err) {
      res.status(401).send(err);
    }
