...
export default class Question {
  ...
  static async insert({ level, questionTitle, questionText, answer, a, b, c, d, explanation, language }) {
    console.log(level, questionTitle, questionText, answer, a, b, c, d, explanation, language)
  ...
  static async findQuestionsByDifficulty(level) {
    const { rows } = await pool.query(
      'SELECT * FROM questions WHERE level = $1', [level]
    );
    return rows.map(row => new Question(row));
  }
}
