...
export default class Response {
  ...
  static async PatchIncorrectAnswerToCorrect(userId, questionId) {
    const { rows } = await pool.query(
      'UPDATE responses SET is_correct = TRUE WHERE user_id = $1 AND question_id = $2 RETURNING *', [userId, questionId]
    );
    return new Response(rows[0]);
  }
}
